...
extern lv_theme_t * lv_theme_material_init(uint16_t hue, lv_font_t *font);
...
int rt_lvgl_init(void)
{
    ...
    thread = rt_thread_create("lv_gui", lv_gui_run, RT_NULL, 30*1024, 7, 10); 
    if(thread == RT_NULL)
    {
        return RT_ERROR;
    }
